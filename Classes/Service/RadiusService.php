...
class RadiusService
{
    ...
    public function getDistance($coordinates1, $coordinates2)
    {
        ...
        $a                 = sin($distanceLatitude / 2) * sin($distanceLatitude / 2)
            + cos($lat1) * cos($lat2) * sin($distanceLongitude / 2) * sin($distanceLongitude
                / 2);
     ...
     * @return array
     */
    public function findAllDatabaseRecordsInRadius($coordinates,
                                                   $maxDistance = 250,
                                                   $tableName = 'pages',
                                                   ...
                                                   $additionalFields = '')
    {
        ...
        $records = $GLOBALS['TYPO3_DB']->exec_SELECTgetRows(
            'uid, '.$distanceSqlCalc.' AS distance'.($additionalFields ? ' , '.$additionalFields
                    : ''), $tableName, $distanceSqlCalc.' < '.$maxDistance,
            '', // group by
            'distance ASC'
        );
        return $records;
